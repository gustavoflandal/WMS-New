...
- **DOC-17 Execução por Tela (§6) — backend completo em 2026-08-25 (Sessão 10E)** — Modo de Execução por armazém e por operação (RN-TEL-010, com a trava que impede concluir num canal tarefa iniciada em outro), permissão própria `TEL.EXECUCAO_TELA` (RN-TEL-012) e `execution_channel` (RD-TEL-004) em tarefas e movimentações. A paridade (RN-TEL-011) já era estrutural: as 8 operações do catálogo RF-TEL-013 sempre tiveram um único serviço de domínio, usado pelo coletor — criar endpoint "de tela" seria o caminho paralelo que a regra proíbe. Correção de raiz no caminho: `wms.app_parameter` não tinha chave única, então os `ON CONFLICT DO NOTHING` de 14 migrations não protegiam nada. Ver `docs/relatorios/SESSAO-10E-relatorio.md`.
- **Próximo: DOC-17 as 8 telas T-P1..T-P8 (frontend)** — último item do DOC-17; consomem os endpoints que já existem, com densidade desktop e o design system. Módulos restantes sem pré-requisito técnico bloqueante entre si: **DOC-13** (integrações), **DOC-09** (faturamento).
