...
- **Próximo: DOC-17 (restante)** — Transcrição (§8: dupla digitação, idempotência, segregação de funções) e Execução por Tela (§6: 8 telas T-P1..T-P8, `execution_channel`). Módulos restantes sem pré-requisito técnico bloqueante entre si: **DOC-13** (integrações), **DOC-09** (faturamento).
