...
- **DOC-17 Parte A (Detalhe de Etapa) completa em 2026-08-25** — contrato único `GET .../fluxo-operacional/:entity/:entityId/steps/:stepCode/detail` (RF-TEL-001/004), os 4 modos Consulta/Execução/Previsão/Bloqueada (RN-TEL-002) e o catálogo de conteúdo por etapa (RF-TEL-003) para os 3 fluxos reais hoje existentes, ver `docs/relatorios/SESSAO-10A-relatorio.md`. Frontend ainda não consome (`FlowTrail.tsx` continua tratando etapa futura como inerte) — fica para uma sessão de frontend dedicada.
- **Próximo: DOC-17 Parte B (10B)** — Formulário de Campo, Transcrição (dupla digitação, idempotência, segregação de funções), 8 telas de execução (T-P1..T-P8). Módulos restantes sem pré-requisito técnico bloqueante entre si: **DOC-13** (integrações), **DOC-09** (faturamento).
