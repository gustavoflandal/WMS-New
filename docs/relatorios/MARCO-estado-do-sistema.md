...
- **DOC-07 9A (núcleo da Logística Reversa) concluído em 2026-08-25** — ver `docs/relatorios/SESSAO-9A-relatorio.md`: Ordem de Devolução, Triagem (matriz RN-REV-021), Destinação com efeito de saldo real e gancho fiscal real (RN-REV-023, reverte `fiscal_allocation` de verdade). **Próximo: DOC-07 9B** (integração real com Gate-in/Portaria — devolução sem agendamento, `REV.SEM_AUTORIZACAO` automático, `RECUSA_ENTREGA` automática — e Recall RF-REV-030), posição 3 do roteiro. A divisão 9A/9B foi um achado de leitura de código na 9A (`DockService`/`GateInService` são hardcoded para `inbound_order`/agendamento), não uma estimativa a priori.
- Módulos restantes após o DOC-07, sem pré-requisito técnico bloqueante entre si: **DOC-13** (integrações), **DOC-09** (faturamento). **DOC-17** (detalhe/execução por tela) depende do DOC-07 completo (9B).
- Para uma demonstração completa do que já funciona: o teste de MARCO em `apps/backend/src/modules/expedicao/__tests__/picking-packing-carregamento.integration.spec.ts` é o roteiro ponta a ponta mais fiel (pedido → COMPLETED) disponível hoje; para a reversa, `apps/backend/src/modules/reversa/__tests__/return-order.integration.spec.ts` cobre o ciclo Ordem→Triagem→Destinação→fiscal.
