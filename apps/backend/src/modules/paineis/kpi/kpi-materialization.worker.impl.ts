// DOC-10 RN-PAI-042 [INVIOLÁVEL] — worker de materialização de KPIs. Lê os
// MESMOS streams Redis events:* que realtime-fanout.worker.impl.ts (RNF-ARQ-033),
// mas com um GRUPO CONSUMIDOR PRÓPRIO ('group:kpi-materialization') — Redis
// Streams suporta múltiplos grupos independentes no mesmo stream por
// natureza; nenhuma mudança no publisher (outbox-publisher.worker.impl.ts)
// foi necessária. Mesmo padrão de ACK-só-após-sucesso, XAUTOCLAIM e DLQ do
// fanout — copiado deliberadamente, não uma dependência entre os dois
// workers (evitar acoplar a materialização de KPI à disponibilidade do
// fanout de tempo real, que tem SLA diferente, RNF-ARQ-088 ≤2s vs KPI sem
// SLA de latência).
...
export interface KpiMaterializationWorkerOptions {
...
export class KpiMaterializationWorkerImpl {
  private readonly logger = new Logger(KpiMaterializationWorkerImpl.name);
  ...
  constructor(
    ...
    options: KpiMaterializationWorkerOptions = {}
  ) {
    this.pollIntervalMs = options.pollIntervalMs ?? 1000;
    this.blockMs = options.blockMs ?? 1000;
    this.minIdleMs = options.minIdleMs ?? 60000;
    this.maxRetries = options.maxRetries ?? 5;
    this.consumerGroup = options.consumerGroup ?? 'group:kpi-materialization';
    this.consumer = options.consumerName ?? `consumer:${process.pid}:${Date.now()}`;
    this.dlqStreamKey = options.dlqStreamKey ?? 'events:dlq:kpi';
  }
  ...
  async start(): Promise<void> {
    await this.init();
    this.logger.log('KPI materialization worker started');
    this.running = true;
  ...
  private async loop(): Promise<void> {
    while (this.running) {
      try {
        ...
        this.logger.error('KPI materialization poll error', error as Error);
  ...
  private async handleMessage(streamKey: string, messageId: string, fields: Record<string, string>): Promise<boolean> {
    try {
      await this.kpiMaterializationService.applyEvent({
      ...
      this.logger.error(`Failed to materialize KPI for ${fields.event_id} on ${streamKey}`, error as Error);
      return false;
    }
  }
  ...
  private async moveToDlq(streamKey: string, messageId: string, fields: Record<string, string>, reason: string): Promise<void> {
    ...
    await this.redisClient.xAck(streamKey, this.consumerGroup, messageId);
    this.logger.warn(`Event ${fields.event_id} moved to KPI DLQ after exceeding ${this.maxRetries} deliveries`);
  }
  ...
  async stop(): Promise<void> {
    ...
    this.logger.log('KPI materialization worker stopped');
  }
