...
describe('Cadastro - unicidade de SKU por cliente', () => {
  ...
  it('SKU duplicado no mesmo cliente e rejeitado; mesmo SKU em cliente diferente e aceito', async () => {
    const clientA = await clientService.create(
      { code: randomClientCode(), legal_name: 'Cliente A SKU', cnpj: generateValidCnpj() },
      SEED_ACTOR_ID
    );
    const clientB = await clientService.create(
      { code: randomClientCode(), legal_name: 'Cliente B SKU', cnpj: generateValidCnpj() },
      SEED_ACTOR_ID
    );
    ...
    const productA = await productService.create(
      { tenant_id: clientA.id, sku, description: 'Produto do cliente A', species_code: 'GERAL', base_uom: 'UN' },
      SEED_ACTOR_ID
    );
    ...
    const productB = await productService.create(
      { tenant_id: clientB.id, sku, description: 'Produto do cliente B', species_code: 'GERAL', base_uom: 'UN' },
      SEED_ACTOR_ID
    );
    ...
    await expect(
      productService.create(
        { tenant_id: clientA.id, sku, description: 'Produto duplicado do cliente A', species_code: 'GERAL', base_uom: 'UN' },
        SEED_ACTOR_ID
      )
    ).rejects.toBeTruthy();
  });
